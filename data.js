...
const CFG = {
  brand: 'Just shop',
  // Логотип. Векторний, зібраний з оригіналу дизайнера — однаково чіткий
  // і в шапці, і на банері. Порожньо — у шапці буде текстовий знак JUST•SHOP.
  logo: 'img/logo.svg',
  ...
  email: 'justshop@example.com',    // ← пошта служби підтримки
  // Продавець — за законом про електронну комерцію й вимогами LiqPay він має
  // бути названий на сайті. Усе це міняється в адмінці («Контакти й підвал»).
  sellerName: 'ФОП Прізвище Імʼя По батькові',
  sellerCode: '0000000000',        // РНОКПП (ідентифікаційний код) ФОП
  sellerAddress: 'Україна, м. Київ, вул. Назва, 1',
...
const FAQ = [
  ...
  { q: 'Як оплачувати?', a: 'Карткою на сайті через LiqPay або накладеним платежем Нової Пошти — при отриманні. Для речей під запит — передоплата 50% за реквізитами в особистих, решта перед відправленням.' },
...
const PAYMENT = [
  { t: 'Карткою на сайті', d: 'Visa, Mastercard, Apple Pay чи Google Pay на захищеній сторінці LiqPay (ПриватБанк). Дані картки вводите на стороні LiqPay — ми їх не бачимо й не зберігаємо.',
    list: ['Замовлення тримаємо за вами 10 хвилин до оплати', 'Відправляємо в день оплати, якщо річ у наявності', 'Повернення коштів — на ту саму картку'] },
  { t: 'Накладений платіж', d: 'Оплата при отриманні на відділенні. Нова Пошта бере власну комісію за переказ грошей — це не наша націнка.',
    list: ['Доступно для замовлень із наявності', 'Для речей під запит — після передоплати 50%'] },
  { t: 'Речі під запит', d: 'Передоплата 50% за реквізитами — надсилаємо в особистих, коли підтвердимо модель і розмір. Решту — перед відправкою з Європи.',
    list: ['Оформлюються в Telegram чи Instagram, не через кошик', 'За потреби даємо чек'] }
];
...
const RETURNS = [
  ...
  { t: 'Повернення коштів', d: 'Не підійшла річ, а обміняти немає на що — повертаємо гроші протягом 7 днів після того, як річ приїде до нас.',
    list: ['Оплату карткою — на ту саму картку через LiqPay', 'Накладений платіж і готівку — переказом на вашу картку', 'Доставку повернення речі належної якості оплачує покупець'] }
];
...
const CONTACTS = [
  ...
  { t: 'Telegram', d: 'Канал із дропами й цінами.', list: [], link: CFG.tg, linkText: 'Перейти в канал' },
